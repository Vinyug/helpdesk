<div class="relative">
    <img class="absolute object-cover object-center w-full xl:h-[500px] lg:h-[450px] h-[400px]" src="assets/images/jumbotron.jpg" alt="image serveur">
    <div class="relative flex justify-center items-center xl:h-[500px] lg:h-[450px] h-[400px] bg-opacity-80 bg-indigo-900">
        <div class="text-white m-3 text-center">
            <h1 class="font-share-tech font-bold text-5xl md:text-6xl lg:text-7xl mb-3">HELPDESK - Axess Informatique</h1>
            <h5 class="text-lg md:text-xl lg:text-2xl mb-4">
                Accéder à la performance, la pérennité et la sécurité pour votre infrastructure.
            </h5>
            <p class="hidden sm:block text-base lg:text-lg mx-auto w-3/4">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Saepe pariatur et similique architecto delectus, ducimus aliquam quos quas nostrum quam dolorum cumque quod, vel eum id! Mollitia, non asperiores.
            </p>
        </div>
    </div>
</div>