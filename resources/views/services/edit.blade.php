        ...
        <form action="{{ route('services.update',$listing->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid sm:grid-cols-2 gap-4 p-4">    
                ...
                <div class="col-span-full">
                    ...
                    <textarea class="custom-input h-80" name="description" id="description" placeholder="Saisir une description">{{ old('description', optional($listing)->description) }}</textarea>
                    @error('description')
                    <div class="custom-error">{{ $message }}</div>
                    @enderror
                </div>
